...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current;
      const needed =
        (cfg.cardWidth as number) +
        Math.abs(cfg.spread as number) * 2 +
        120;
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const step = clamp(
        delta / ((cfg.cardWidth as number) * 0.9),
        -0.6,
        0.6
      );
      posRef.current += step;
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const stepPx = Math.max(
        (cfg.cardWidth as number) * 0.55 * scaleRef.current,
        40
      );
  ...
  return (
    <div
      ...
      style={
        { "--dc-perspective": `${perspective}px` } as React.CSSProperties
      }
      role="group"
